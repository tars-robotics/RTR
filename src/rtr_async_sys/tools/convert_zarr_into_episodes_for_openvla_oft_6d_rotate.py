"""
10d action: xyz+6drotate+gripper
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Convert RDP zarr replay buffer into per-episode .npy files "
                    "(10d action: xyz+6drotate+gripper) for openvla-oft fine-tuning.",
    )
...
for i in tqdm.tqdm(range(num_episodes)):
    ...
    for offset in range(end - start):
        ...
        action = episode_actions[offset] #convert_10d_action_to_7d_action(np.asarray(actions[step], dtype=np.float32)).astype(np.float32)
        state = convert_9d_state_to_8d_state(episode_states[offset]).astype(np.float32)
        # print(state.shape)
        episode_list.append({
            "image": episode_images[offset], # (240, 320, 3), int
            "action": action, # (10), units: meters
            "state": state,
