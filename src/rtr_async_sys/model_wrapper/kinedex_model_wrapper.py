...
class KinedexModelWrapper(AbsModelWrapper):
    def __init__(
            self, 
        ...
        assert isinstance(self.policy, DiffusionUnetImagePolicy), "model should be DiffusionUnetImagePolicy in KinedexModelWrapper"
    ...
    def predict_action_chunk(self, obs_dict: Dict[str, torch.Tensor]) -> np.ndarray:
        ...
        # ========== copy from kinedex_image_policy.DiffusionUnetImagePolicy
        # result = {
...
@hydra.main(
    config_path="../configs/user/model_wrapper",
    config_name="kinedex_wrapper",
    version_base=None
)
def main(cfg: DictConfig):
    if isinstance(cfg, str):
        cfg = OmegaConf.load(cfg)
    model_wrapper = hydra.utils.instantiate(cfg)
    import time
    time.sleep(5)
    print(model_wrapper)

if __name__ == '__main__':
    # cfg = OmegaConf.load("src/rtr_async_sys/configs/user/model_wrapper/kinedex_wrapper.yaml")
