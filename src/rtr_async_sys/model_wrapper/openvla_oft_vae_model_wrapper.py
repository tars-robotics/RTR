...
from rtr_async_sys.models.reactive_diffusion_policy.model.vae.model import Openvla_oft_VAE
...
class OpenvlaOftVaeModelWrapper(AbsModelWrapper):
    def __init__(
            self, 
            ...
            action_quant:bool = False
        ):
        super().__init__(model, device, ckpt_path, return_raw_action, need_refine, compress_obs=compress_obs, build_policy=build_policy)
        

        assert isinstance(self.policy, Openvla_oft_VAE), "model should be OpenvlaOftVAE in OpenvlaOftVaeModelWrapper"
        self.policy.to(device)
        self.input_key_list = ['action']
        self.dataset_statistics_path = dataset_statistics_path

        self.policy._load_dataset_statistics(self.dataset_statistics_path)
        self.action_quant = action_quant
    ...
    def predict_action_chunk(self, obs_dict: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        """
        obs_dict = self.preprocess_obs(obs_dict)# after process, get torch.tensor
        action = obs_dict['action']
        naction = self.policy.normalize_from_dataset(action)

        
        with torch.no_grad():
            if self.action_quant:
                naction_chunk = self.policy.encode_then_decode_action_quant(naction)
            else:
                naction_chunk = self.policy.encode_then_decode(naction)

        action_chunk = self.policy.denormalize_from_dataset(naction_chunk)
...
@hydra.main(config_path="../configs/user/model_wrapper", config_name="openvla_oft_vae_wrapper", version_base=None)
def cli_main(cfg: Union[DictConfig, str]):
    if isinstance(cfg, str):
        cfg = OmegaConf.load(cfg)
    vae_model_wrapper = hydra.utils.instantiate(cfg)
    print(vae_model_wrapper)
    print(vae_model_wrapper.policy._get_action_stats_torch('cpu',torch.float32))
