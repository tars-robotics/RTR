...
from tactile_generation_policy.common.replay_buffer import ReplayBuffer
from tactile_generation_policy.common.sampler import (
...
# from tactile_generation_policy.real_world.real_world_transforms import RealWorldTransforms
...
class RealImageTactileDataset(BaseImageDataset):
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        if self.relative_action:
            relative_data_dict = {key: list() for key in (self.lowdim_keys + ['action']) if ('tcp_pose' in key and 'tactile' not in key) or 'action' in key}
            for key in relative_data_dict.keys():
                relative_data_dict[key] = self.replay_buffer[key]
