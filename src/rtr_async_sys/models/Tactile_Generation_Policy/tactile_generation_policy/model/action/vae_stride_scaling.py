...
class ActionVAE(nn.Module):
    ...
    def calculate_loss(self, state):
        ...
        # ===== Scale loss by stride =====
        use_step_factor = True

        state_reshape = state.reshape(state.shape[0], self.horizon, -1).detach() # N, T, A
        diff = state_reshape[:, 1:, :] - state_reshape[:, :-1, :]   # (N, T-1, A)

        # step = diff.abs().sum(dim=-1)    # (N, T-1) # l1
        step = diff.norm(dim=-1)          # (N, T-1) # l2

        step = step.sum(dim=-1)           # (N)
        self.max_step = max(self.max_step, step.max())
        step_factor = self.max_step / step # (N)
        # print(f"======= max_step is {self.max_step}, \n step is {step} \n step_factor is {step_factor} ======")
        
        if use_step_factor:
            encoder_loss = (state - dec_out).abs() # (N, T*A)
            encoder_loss = (encoder_loss * step_factor.unsqueeze(1)).mean()
        else:
