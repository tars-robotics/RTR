...
def torch_upsample_with_anchor(action_chunk: torch.Tensor, T: int, r: int) -> torch.Tensor:
    ...
    Return: [B, T, D] (interpolated back to the original length), with out[:, xp[k], :] == action_chunk[:, k, :].
    """
    ...
    xp = torch.arange(0, T, r, device=device)  # [N] (usually equal to N)
    # Optional: keep xp length aligned with N for non-divisible boundaries.
    if xp.numel() != N:
        # Truncate/align using the N dimension of action_chunk.
        xp = xp[:N]
    ...
    # 2) Overwrite anchors so out matches the input exactly at xp positions.
...
# Train with the DP train_cfg; the key point is to use the non-latent dataset.
class InterpolateDiffusionUnetImagePolicy(DiffusionUnetImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # Interpolate back to original_shape.
        # print("="*100)
        ...
        # get action
        # start = To - 1: To-1is a hack, because it drops n_obs_steps actions, though it should only drop n_obs_steps - 1 actions; in the dataset, obs[0] corresponds to action[0] as the next state
        start = (To-1)*self.image_downsample_ratio
        end = start + self.n_action_steps
        action = action_pred[:,start:end]
        
        result = {
            'action': action,# future action
            'action_pred': action_pred,# full action prediction
        }
        return result
