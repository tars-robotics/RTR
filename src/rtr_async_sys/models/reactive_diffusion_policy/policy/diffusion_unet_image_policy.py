...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            # nobs is the current observation; do not truncate it with reverse_length.
        ...
        # With image_downsample_ratio, one observation maps to image_downsample actions; n_obs_steps - 1 are past observations, so (To - 1) * image_downsample_ratio are past actions.
        start = (To-1)*self.image_downsample_ratio
        end = start + self.n_action_steps
        action = action_pred[:,start:end]
        action_reverse = torch.flip(action_pred[:, :self.reverse_length], dims=[1])  # action is a torch.Tensor, so [::-1] is not valid here.
        # action_reverse = action_pred[:, 0:self.reverse_length][::-1] # action_reverse[0] is closest to the current obs; action_reverse[-1] is farthest.
        
        result = {
            'action': action,# future action
            'action_pred': action_pred,# full action prediction
            'action_reverse': action_reverse  # Past actions
        }
        return result
