...
class RealImageTactileDataset(BaseImageDataset):
    def __init__(self,
        ...
        :type shape_meta: dict
        :type dataset_path: str
        :param image_downsample_ratio: when images are sampled at a lower rate than states but upsampled in zarr to the same rate, downsample them before use.
        """
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.lowdim_keys:
            if 'wrt' not in key:
                # IMPORTANT: observations such as images and states share the same policy horizon, so they must be aligned.
