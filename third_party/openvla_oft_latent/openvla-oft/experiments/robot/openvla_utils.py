...
def normalize_proprio(proprio: np.ndarray, norm_stats: Dict[str, Any]) -> np.ndarray:
    ...
    if ACTION_PROPRIO_NORMALIZATION_TYPE == NormalizationType.NORMAL:
        ...
        mean = np.asarray(norm_stats["mean"], dtype=proprio.dtype)
        std = np.asarray(norm_stats["std"], dtype=proprio.dtype)
        mask = np.asarray(
            norm_stats.get("mask", np.ones_like(mean, dtype=bool)),
            dtype=bool,
        )

        normalized_proprio = np.where(
            mask,
            (proprio - mean) / (std + 1e-8),
            proprio,
        )
