...
def normalize_proprio_not_normalize_action(
    traj: Dict,
    metadata: Dict,
    normalization_type: NormalizationType,
):
    """
    Only supports NormalizationType.NORMAL.
    Only normalizes proprio (traj key: 'observation/proprio').
    Does NOT normalize action.
    """
    if normalization_type != NormalizationType.NORMAL:
        raise ValueError(f"Only NormalizationType.NORMAL is supported, got {normalization_type}")

    key = "proprio"
    traj_key = "observation/proprio"
...
def normalize_action_and_proprio_noop_for_vae(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    """
    No-op (skip normalization).
    No-op version: does NOT change values, but still executes the same selective_tree_map
    passes to preserve any non-obvious side effects (structure/dtype checks, etc.).
    """
    keys_to_normalize = {"action": "action", "proprio": "observation/proprio"}

    if normalization_type == NormalizationType.NORMAL:
        for key, traj_key in keys_to_normalize.items():
            _ = metadata[key].get("mask", tf.ones_like(metadata[key]["mean"], dtype=tf.bool))
            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: x,  # identity
            )
        return traj
        ...
        for key, traj_key in keys_to_normalize.items():
            if normalization_type == NormalizationType.BOUNDS:
                _low = metadata[key]["min"]
                _high = metadata[key]["max"]
            else:  # NormalizationType.BOUNDS_Q99
                _low = metadata[key]["q01"]
                _high = metadata[key]["q99"]

            _ = metadata[key].get("mask", tf.ones_like(metadata[key]["min"], dtype=tf.bool))

            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: x,  # identity (no clip/scale)
            )

            # Intentionally do NOT apply zeros_mask (no value change).
        return traj
...
def get_dataset_statistics(
    ...
    if save_dir is not None:
        ...
        path = local_path
    print(f"path is {path}, local_path is {local_path}, vae is {vae}")

    # check if cache file exists and load
    if tf.io.gfile.exists(path):
        print(f"load from {path}")
        print("DEBUG NOT LOAD!!!")
    ...
    for traj in tqdm(dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None):
        actions.append(traj["action"])# T,A
        proprios.append(traj["proprio"])

        if vae is not None:
            with torch.no_grad():
                act = np.asarray(traj["action"])
                act_t = torch.tensor(act[None, :], dtype=torch.float32)  # [1, T, A] e.g. [1, 331, 10]
                T_for_vae = vae.input_dim_h  # e.g. 48

                # TODO: reshape act_t to [B, T_for_vae, A]; drop trailing timesteps that don't divide evenly
                T_total = act_t.shape[1]
                A = act_t.shape[2]
                B = T_total // T_for_vae
                if B == 0:
                    # This trajectory is too short to form a complete chunk; skip it
                    continue

                act_t = act_t[:, : B * T_for_vae, :]          # [1, B*T_for_vae, A]  drop the trailing portion
                act_t = act_t.reshape(B, T_for_vae, A)        # [B, T_for_vae, A]

                act_t = vae.normalize_from_dataset(act_t)
                na = vae.encode_to_latent(act_t).detach().cpu().reshape(-1, A).numpy()  # [T,A]
                nactions.append(na)            # Store uniformly as [B*T_for_vae, Z] for easy concat-based statistics
    ...
    if vae is not None:
        print("vae is not none")
        nactions = np.concatenate(nactions)  # [sumT, Z]
    ...
    if vae is not None:
        metadata = {
            ...
            "naction": {
                    "mean": nactions.mean(0).tolist(),
                    "std": nactions.std(0).tolist(),
                    "max": nactions.max(0).tolist(),
                    "min": nactions.min(0).tolist(),
                    "q01": np.quantile(nactions, 0.01, axis=0).tolist(),
                    "q99": np.quantile(nactions, 0.99, axis=0).tolist(),
            },
...
def save_dataset_statistics(dataset_statistics, run_dir):
    ...
    with open(out_path, "w") as f_json:
        for _, stats in dataset_statistics.items():
            ...
            # >>> ADD: naction
            if "naction" in stats:
                for k in stats["naction"].keys():
                    if isinstance(stats["naction"][k], np.ndarray):
                        stats["naction"][k] = stats["naction"][k].tolist()
