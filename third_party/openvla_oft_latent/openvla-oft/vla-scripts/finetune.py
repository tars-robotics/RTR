...
from prismatic.vla.datasets import RLDSBatchTransform,  RLDSDataset_with_vae
...
class FinetuneConfig:
    ...
    vae_ckpt_path:str = 'data/ckpts/vase_sponge_test1_60hz/ckpts_abs/openvla_oft/vae/horizon48_compress4_n_embed_10/latest.ckpt'
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if should_log_l1_loss:
            ...
            if use_vae:
                ...
                original_predicted_actions = vae.decode_from_latent(predicted_actions.to(vae_device).to(torch.float32))
                ...
                recover_original_ground_truch_actions = vae.decode_from_latent(compress_original_ground_truch_actions)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    train_dataset = RLDSDataset_with_vae(
        cfg.data_root_dir,
        cfg.dataset_name,
        batch_transform,
        ...
        vae_dataset_statistics_path=cfg.vae_dataset_statistics_path
    )
    use_vae = train_dataset.use_vae
    if use_vae:
        vae = train_dataset.vae
    if cfg.use_val_set:
        val_dataset = RLDSDataset_with_vae(
            cfg.data_root_dir,
            cfg.dataset_name,
            batch_transform,
            resize_resolution=tuple(vla.module.config.image_sizes),
