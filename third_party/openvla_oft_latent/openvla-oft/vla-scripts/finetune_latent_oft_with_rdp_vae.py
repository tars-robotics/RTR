...
from prismatic.vla.datasets import RLDSBatchTransform,  RLDSDataset_with_rdp_vae
...
class FinetuneConfig:
    ...
    vae_ckpt_path:str = 'data/ckpts/vase_sponge_test1_60hz/ckpts_abs/rdp_vae/n_embed_10/horizon48/latest.ckpt'
...
def run_forward_pass(
    ...
    original_ground_truch_actions = batch["original_actions"].to(device_id).to(torch.bfloat16)
    ...
    # Compute metrics for continuous action representations (L1 regression | diffusion)
        ...
        if should_log_l1_loss:
            ...
            all_actions_l1_loss =  torch.nn.L1Loss()(ground_truth_actions, predicted_actions)
            original_ground_truch_actions = original_ground_truch_actions
            ...
            if use_vae:
                ...
                if normalize_latent:
                    ...
                    # for debug
                    # predicted_actions = vae.denormalize_from_dataset(ground_truth_actions.to(vae_device).to(torch.float32), is_latent=True)
                    predicted_actions = vae.denormalize_from_dataset(predicted_actions.to(vae_device).to(torch.float32), is_latent=True)

                original_predicted_actions = vae.decode_from_latent(predicted_actions.to(vae_device).to(torch.float32)).to(vae_device)
                original_ground_truch_actions = original_ground_truch_actions.to(vae_device).to(torch.float32)
                original_action_l1_loss = torch.nn.L1Loss()(original_ground_truch_actions, original_predicted_actions)
                # import pdb;pdb.set_trace()
                compress_original_ground_truch_actions = vae.encode_to_latent(original_ground_truch_actions)
                recover_original_ground_truch_actions = vae.decode_from_latent(compress_original_ground_truch_actions).to(vae_device)
                reconstruct_original_action_l1_loss = torch.nn.L1Loss()(original_ground_truch_actions, recover_original_ground_truch_actions)
                # l1loss = torch.mean(torch.abs(recover_original_ground_truch_actions - original_ground_truch_actions))
                # print(f"l1loss is {l1loss}")
                ...
                metrics.update(
                    {
                        "curr_action_l1_loss": curr_action_l1_loss.item(),
                        "next_actions_l1_loss": next_actions_l1_loss.item(),
                        "all_actions_l1_loss": all_actions_l1_loss.item(),
                        "original_action_l1_loss": original_action_l1_loss.item(),
                        "reconstruct_original_action_l1_loss": reconstruct_original_action_l1_loss.item()
                    }
                )
            else:
                metrics.update(
...
def _to_jsonable(x):
    """Best-effort convert common ML values to JSON-serializable."""
...
def log_metrics_to_wandb(
    ...
    step: int,
    wandb_entity,
    json_log_path: Optional[str] = None,   # e.g. "runs/exp1/metrics.jsonl"
) -> None:
    """
    Log metrics to Weights & Biases and (optionally) to a local JSONL file.

    Args:
        metrics: Dictionary of metrics to log (may contain torch/numpy values).
        prefix: Prefix for metric names.
        step: Training step.
        wandb_entity: W&B run/object that has .log(...)
        json_log_path: If provided, append logs to this JSONL file.
    ...
    if json_log_path:
        record = {
            ...
            "step": int(step),
            "prefix": prefix,
            "metrics_raw": _to_jsonable(metrics),
            "metrics_wandb_names": _to_jsonable(log_dict),
        }
        _append_jsonl(Path(json_log_path), record)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Do not append a timestamp; multi-process runs can get out of sync and produce mismatched names
    # from datetime import datetime
    ...
    print(f"len bins is {len(vla.bins)}")
    # import pdb;pdb.set_trace()
    ...
    train_dataset = RLDSDataset_with_rdp_vae(
        cfg.data_root_dir,
        cfg.dataset_name,
        batch_transform,
        ...
        use_vae=cfg.use_vae,
        t_downsample_ratio=cfg.t_downsample_ratio,
        horizon=cfg.horizon,
        n_embed=cfg.n_embed,
        vae_config_path=cfg.vae_config_path,
        ckpt_path=cfg.vae_ckpt_path,
        vae_latent_dataset_statistics_path=cfg.vae_latent_dataset_statistics_path,
    )
    use_vae = train_dataset.use_vae
    if use_vae:
        vae = train_dataset.vae
    if cfg.use_val_set:
        val_dataset = RLDSDataset_with_rdp_vae(
            cfg.data_root_dir,
            cfg.dataset_name,
            batch_transform,
            resize_resolution=tuple(vla.module.config.image_sizes),
    ...
    if use_vae:
        recent_metrics = {
            ...
            "all_actions_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
            "original_action_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
            "reconstruct_original_action_l1_loss": deque(maxlen=cfg.grad_accumulation_steps),
        }
    else:
        recent_metrics = {
            "loss_value": deque(maxlen=cfg.grad_accumulation_steps),
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for batch_idx, batch in enumerate(tqdm.tqdm(dataloader, desc="Training", leave=False)):
            # print(batch.keys())
            # import pdb;pdb.set_trace()
            ...
            if distributed_state.is_main_process and log_step % cfg.wandb_log_freq == 0:
                log_metrics_to_wandb(
                    ...
                    log_step, 
                    wandb,
                    json_log_path=os.path.join(run_dir, 'logs.jsonl')
                )
            ...
            progress.set_description(
                f"Step {log_step}/{cfg.max_steps} | Batch {batch_idx}/{len(dataloader)} | Loss: {normalized_loss.item():.4f}"
            )
