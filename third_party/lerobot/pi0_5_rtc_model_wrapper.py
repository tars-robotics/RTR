...
class PI05RTCModelWrapper(AbsModelWrapper):
    """
    ...
    def predict_action_chunk(self, obs_dict: Dict[str, np.ndarray]) -> np.ndarray:
        ...
        with torch.no_grad():
            if not self.openloop_eval:
                ...
                this_x = predict_actions[0,0,0]
                if (self.prev_chunk_left_over is not None) and (self.prev_chunk_left_over[0,-1,0] < 0 and this_x > 0):
                    print("============================swith episode==========================")
                    self.prev_chunk_left_over = None
...
def main(cfg: DictConfig):
    ...
    model_wrapper:PI05RTCModelWrapper = hydra.utils.instantiate(cfg)
    print(model_wrapper)
