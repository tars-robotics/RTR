...
dataset="<your_username>/xarm_write_board_state9_15hz"
...
world_size=1 
run_id=train_logl1_write_board_15hz_world${world_size}_steps${steps} # pi05_training
output_dir="outputs/pi05_training_logl1_write_board_15hz_steps${steps}_world${world_size}"


# rm -rf ${output_dir}

# CUDA_VISIBLE_DEVICES=0,1,2,3 python scripts/codes/train_pi0.5/lerobot_train.py \
TORCHDYNAMO_DISABLE=1 CUDA_VISIBLE_DEVICES=5 accelerate launch --num_processes=${world_size} scripts/codes/train_pi0.5/lerobot_train.py \
    ...
    --policy.repo_id="<your_username>/xarm_write_board_15hz_pi0.5" \
