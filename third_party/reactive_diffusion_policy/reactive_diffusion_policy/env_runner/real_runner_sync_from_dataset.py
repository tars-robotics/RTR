"""
Run from a dataset for visualization / debugging. Mainly modifies RealRobotEnv.
Synchronous flow: obtain 2 observations at a time, feed them to the slow model;
the slow model returns an action chunk, then the fast model uses the entire
chunk to infer the actual action. Since there is no latency, `self.latency_step`
is set to 0.
"""
...
class RealRunner:
    def __init__(self,
        ...
        # self.save_tac_dir = "./data/tactile_data_save"
    ...
    def post_process_action(self, action: np.ndarray) -> Tuple[np.ndarray, bool]:
        ...
        1. Handle rotation: for 10-dim actions, convert the 6D rotation to 3D Euler.
        2. Clip.
        3. Handle gripper info.
        4. Concatenate the actions for both arms.
        """
        ...
        if action.shape[-1] == 4:
            ...
            right_action = np.concatenate([right_action_6d, action[:, 7][:, np.newaxis],
                                           np.zeros((action.shape[0], 1))], axis=1)
        elif action.shape[-1] == 10:  # 6 -> 8: one extra may be grasp flag, the other gripper width
    ...
    def action_command_thread(self, policy: Union[DiffusionUnetImagePolicy], stop_event):
        ...
        while len(self.tcp_ensemble_buffer.actions) != 0:
            ...
            if self.use_latent_action_with_rnn_decoder:
                ...
                # longer_extended_obs_step = 2  # the current dataset env only supports obs_step == 2
                ...
                ###### tactile saving
                # Save left_gripper1_marker_offset_emb to a local npy file, numbered starting from 0000.
                ...
                #     # Count existing npy files in the directory; name files like left_gripper1_marker_offset_emb_0000.npy.
    ...
    def run(self, policy: Union[DiffusionUnetImagePolicy], plot_step_num=2, save_path="data/outputs/vis_outputs/plot_actions.pkl"):
        """
        Optionally accept a `plot_step_start` argument to control where plotting starts.
        """
        if self.use_latent_action_with_rnn_decoder:
        ...
        try:
            ...
            for i in tqdm.tqdm(range(plot_step_num)):
                ...
                # get transformed real obs dict (mainly handles images)
                ...
                # Run inference and update the queue on every step.
                with torch.no_grad():
                    if self.use_latent_action_with_rnn_decoder:
                        logger.debug(f"inference step: steps_per_inference is {steps_per_inference}")
                        # recover the action latent from noise conditioned on observations
                ...
                logger.info(f"update: tcp_action_update_interval is {self.tcp_action_update_interval}")
                if self.use_latent_action_with_rnn_decoder:
