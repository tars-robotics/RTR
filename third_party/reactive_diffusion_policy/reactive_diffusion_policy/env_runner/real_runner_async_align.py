...
class RealRunner:
    ...
    def action_command_thread(self, policy: Union[DiffusionUnetImagePolicy], stop_event):
        while not stop_event.is_set():
            ...
            # precise_sleep(max(0., self.control_interval_time - (cur_time - start_time)))
            self.action_step_count += 1
            self.tcp_ensemble_buffer.update(self.env)
            self.gripper_ensemble_buffer.update(self.env)
    ...
    def run(self, policy: Union[DiffusionUnetImagePolicy]):
        ...
        try:
            time.sleep(5)
            while True:
                if self.tcp_ensemble_buffer.need_update() == False:
                    continue
                ...
                start_time = time.time()
                # get obs
                obs, inference_timestep = self.tcp_ensemble_buffer.get_latest_obs_timestep()
                if obs == None:
                ...
                # run policy
                ## TODO: this extra inference can be removed
                with torch.no_grad():
                    print("-----------------------inference-----------")
                    if self.use_latent_action_with_rnn_decoder:
                        ...
                        action_dict = policy.predict_action(obs_dict)
                    # inject a large delay to test async behaviour
                    time.sleep(2)
                self.latent_cnt += 1
                ...
                self.tcp_ensemble_buffer.add_action(tcp_action, inference_timestep)
                ...
                self.gripper_ensemble_buffer.add_action(gripper_action, inference_timestep)
