...
class LatentChunkDiffusionUnetImagePolicy(DiffusionUnetImagePolicy):
    """
    Use VAE for compression and decompression. The use_rnn_decoder case (i.e., the case that needs a refiner) is not handled here.
    """
    def __init__(self,
                 at: VAE,
    ...
    def predict_action(self,
        ...
        single_latent_dim = latent_dim // self.latent_horizon  # latent_horizon latents total; each must be decoded separately
        ...
        if return_latent_action:
            ...
            # print(action_pred)  # along the original_horizon axis all latents are identical -- the decoder is responsible for turning the identical chunk into per-timestep actions.
        else:
            ...
            naction_pred = nsample[..., :Da]  # affects the last dim
