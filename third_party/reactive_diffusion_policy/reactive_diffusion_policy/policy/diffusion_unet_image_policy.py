...
class DiffusionUnetImagePolicy(BaseImagePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if self.obs_as_global_cond:
            ...
            # nobs is the current observation; do NOT slice it with reverse_length
        ...
        action_reverse = torch.flip(action_pred[:, :self.reverse_length], dims=[1])  # action is a torch.Tensor; [::-1] is not allowed
        # action_reverse = action_pred[:, 0:self.reverse_length][::-1]  # action_reverse[0] is closest to the current obs; action_reverse[-1] is the furthest.
        
        result = {
            'action': action,                  # future actions
