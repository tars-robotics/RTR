...
class ActionVAE(nn.Module):
    ...
    The encoder and decoder both consume / produce tensors of shape [B, T, A].
    The latent dim is set to action_dim (A) so the model can plug directly into the existing policy.
    Normalization happens inside the model; call `set_normalizer` before training.
    """
    def __init__(
        self,
        input_dim=10,
        ...
        """Initialize ActionVAE with encoder/decoder UNets and quantization layers."""
        super().__init__()
...
if __name__ == '__main__':
    ...
    normalizer_path = "./data/outputs/normalizer.pkl"
