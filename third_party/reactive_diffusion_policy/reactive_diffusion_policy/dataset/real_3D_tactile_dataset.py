...
from reactive_diffusion_policy.common.action_utils import absolute_actions_to_relative_actions, get_inter_gripper_actions, pose_3d_9d_to_homo_matrix_batch
from reactive_diffusion_policy.real_world.real_world_transforms import RealWorldTransforms

def dbg_stats(t, name=''):
    """Supports both torch & numpy."""
...
class RealImageTactileDataset(BaseImageDataset):
    def __init__(self,
        ...
        # if n_obs_steps is not None:
            ...
            # for key in pcd_keys + lowdim_keys:
        ...
        # Load initial tactile mesh points
        self.gripper1_tac_init = np.load(os.path.join(dataset_path, 'gripper1_tac_init.npy'))
        self.gripper2_tac_init = np.load(os.path.join(dataset_path, 'gripper2_tac_init.npy'))
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for key in self.pcd_keys:
            ...
            obs_dict[key] = data[key][T_slice][::-obs_downsample_ratio][::-1].astype(np.float32)
            # T,C,H,W
            # save ram
            if key not in self.pcd_keys:
                del data[key]
        ...
        if self.relative_action:
            ...
            if self.relative_tcp_obs_for_relative_action:
                ...
                # add and transform tactile points to base frame 

        if 'left_robot_tcp_pose' in obs_dict:
            tcp_pose = obs_dict['left_robot_tcp_pose']
            T = tcp_pose.shape[0]
            # (T, 4, 4)
            tcp_pose_mat = pose_3d_9d_to_homo_matrix_batch(tcp_pose)

            for gripper_idx, tac_init in [(1, self.gripper1_tac_init), (2, self.gripper2_tac_init)]:
                tactile_key = f'left_gripper{gripper_idx}_tactile'
                # (T, 700, 3)
                tactile_force_data = data[tactile_key][T_slice][::-obs_downsample_ratio][::-1].astype(np.float32)
                
                # (700, 3) -> (T, 700, 3)
                tactile_points_sensor = np.tile(tac_init, (T, 1, 1))

                # Add a homogeneous coordinate
                # (T, 700, 4)
                ones = np.ones_like(tactile_points_sensor[..., :1])
                tactile_points_sensor_homo = np.concatenate([tactile_points_sensor, ones], axis=-1)

                # Transform points
                # (T, 1, 4, 4) @ (T, 700, 4, 1) -> (T, 700, 4, 1)
                transformed_points_homo = np.expand_dims(tcp_pose_mat, 1) @ np.expand_dims(tactile_points_sensor_homo, -1)
                
                # (T, 700, 3)
                transformed_points = transformed_points_homo.squeeze(-1)[..., :3]

                # Concatenate transformed points and tactile force data
                # (T, 700, 6)
                combined_data = np.concatenate([transformed_points, tactile_force_data], axis=-1)
                obs_dict[tactile_key] = combined_data.astype(np.float32)
