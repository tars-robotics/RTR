...
class RealImageTactileDataset(BaseImageDataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # in __getitem__, where point clouds were originally stored
        for key in self.pcd_keys:          # existing loop
            if key == 'global_pts':        # transpose only this key
                # from [T,3,8192] -> [T,8192,3]; if no T dim, [3,8192] -> [8192,3]
