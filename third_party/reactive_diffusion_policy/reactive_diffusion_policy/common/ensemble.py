...
class EnsembleBuffer:
    ...
    def add_action(self, action, timestep):
        ...
        # while idx + horizon - 1 >= len(self.actions):  # add the entire chunk starting from step idx
        #     self.actions.append([])
        ...
        print(f"Appended new action to queue. horizon={action.shape[0]}, timestep={timestep}, actions_start_timestep={self.actions_start_timestep}, len(actions)={len(self.actions)}")
