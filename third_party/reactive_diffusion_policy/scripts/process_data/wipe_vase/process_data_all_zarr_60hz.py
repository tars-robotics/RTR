...
def process_one_episode(data_path, policy, vis_save_path=None, save_camera_vis=False, save_tactile_vis=False):
    ...
    try:
        ...
        if policy == 'dp_zarr':
            ...
            start_frame = np.where(np.diff(states_arrays[:, 2]) > 1)[0][0] # action is 60Hz now, so threshold lowered to 0.5 # crop the leading frames
            end_frame = np.argmin(states_arrays[:, 0]) + 3 # take a few extra trailing frames
